...
test.describe('Auth Screenshots', () => {
  ...
  test('LoginView — Error: empty fields submitted', async ({ page }) => {
    ...
    await expect(page.locator('.errors')).toContainText('Email is required')
    await expect(page.locator('.errors')).toContainText('Password is required')
  ...
  test('SignupView — Error: passwords do not match', async ({ page }) => {
    ...
    await page.fill('#signup-confirm-password', 'different')
    await page.click('button[type="submit"]')
    await expect(page.locator('.errors')).toBeVisible()
    await expect(page.locator('.errors')).toContainText('Passwords do not match')
  ...
  test('SignupView — Error: short password', async ({ page }) => {
    ...
    await page.fill('#signup-confirm-password', 'short')
    await page.click('button[type="submit"]')
    await expect(page.locator('.errors')).toBeVisible()
    await expect(page.locator('.errors')).toContainText('Password must be at least 8 characters')
